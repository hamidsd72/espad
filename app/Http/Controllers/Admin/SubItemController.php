...
class SubItemController extends Controller {
    public function controller_title($type) {
        if ($type == 'sum') {
            return 'آیتم های صفحه کسب و کار نوین';
        } elseif ('single') {
            return 'آیتم صفحه کسب و کار نوین';
        }
    }
    ...
    public function index() {
        ...
        return view('admin.subItems.show', compact('items'), ['title1' => $this->controller_title('single'), 'title2' => $this->controller_title('sum')]);
    }

    public function show($id) {
        $items = Item::where('page_name',$id)->orderByDesc('id')->paginate($this->controller_paginate());
        return view('admin.subItems.show', compact('id','items'), ['title1' => $this->controller_title('single'), 'title2' => $this->controller_title('sum')]);
    }

    public function create($id=null) {
        return view('admin.subItems.create', compact('id'),['title1' => $this->controller_title('single'), 'title2' => $this->controller_title('sum')]);
    }
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            $item->save();
            return redirect()->route('admin.sub-item.show',$request->page_name)->withInput()->with('flash_message', 'آیتم با موفقیت ایجاد شد.');
        } catch (\Exception $e) {
    ...
    public function edit($id) {
        $item = Item::find($id);
        return view('admin.subItems.edit', compact('item'), ['title1' => $this->controller_title('single'), 'title2' => $this->controller_title('sum')]);
    }
    ...
    public function update(Request $request, $sub_service) {
        ...
        try {
            ...
            $item->update();
            return redirect()->route('admin.sub-item.show',$item->page_name)->withInput()->with('flash_message', 'موفقیت ویرایش شد.');
        } catch (\Exception $e) {
