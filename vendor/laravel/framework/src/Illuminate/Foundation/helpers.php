...
if (!function_exists('call_me')) {
    function call_me($consultant_mobile,$user_mobile,$min)
    {
        $sett=Setting::first();
        if(!$sett)
        {
        ...
        $email = $sett->call_user;
        $pass = $sett->call_pass;
        $siteurl = 'manabourse.com';
        $customer = $consultant_mobile;
        $drivers = $user_mobile;
//        دقیقه و اختیاری است.
        $maxtalktime = $min;
// صوت اختصاصی پخش میشود
//        $ivrfile = 'custom/987';
// هیچ صوتی پخش نشود.
//        $ivrfile = url('source/asset/call_me.mp3');
        $ivrfile = 'custom/none';
        $myURL = "https://portal.callmee.org/my/api".$sett->call_code.".php?" .
            "email=" . $email . "&" .
            "pass=" . $pass . "&" .
            "siteurl=" . $siteurl . "&" .
            "customer=" . $customer . "&" .
        ...
        $end=file_get_contents($myURL);

        $end=json_decode($end,true);
        return $end;
    }
}
if (!function_exists('call_me_reply')) {
    function call_me_reply($code)
    {
        $sett=Setting::first();
        if(!$sett)
        {
        ...
        $pass = $sett->call_pass;
        $file_call = $code;
        $myURL="https://portal.callmee.org/my/getCDR.php?" .
            "email=" . $email . "&" .
            "pass=" .$pass ."&" .
            "callfile=" .$file_call;
