        ...
        <div class="card card-primary card-outline">
            <div class="card-body box-profile">
                <form action="{{ route('admin.item.store') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        @if ($id??'')
                            {{ Form::hidden('page_name', $id, array('class' => 'form-control')) }}
                        @else
                            <div class="col-lg-12">
                                <div class="form-group text-danger">
                                    {{ Form::label('page_name', 'نام صفحه ') }}
                                    <select name="page_name" class="form-control select2">
                                        @foreach (\App\Model\ServiceCat::where('status','active')->get() as $key => $item)
                                            <option value="{{$item->slug}}" @if($key==0) selected @endif>{{$item->title}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
