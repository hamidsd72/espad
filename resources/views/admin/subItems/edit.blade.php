    ...
    <section class="content">
        <div class="card card-primary card-outline">
            <div class="card-body box-profile">
                {{ Form::model($item,array('route' => array('admin.sub-item.update', $item->id), 'method' => 'PATCH', 'files' => true)) }}
                    <div class="row">
                        ...
                        <div class="col-lg-6">
                            <div class="form-group text-danger">
                                ...
                                <select name="service_id" class="form-control select2">
                                    @foreach ( \App\Model\Service::where('status','active')->get() as $service)
                                        <option value="{{$service->id}}" @if($service->id==$item->service_id) selected @endif>{{$service->title.' - '}}{{$service->category()?$service->category()->title:'________'}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-12">
                            <div class="form-group">
                                ...
                                {{ Form::textarea('text',$item->text, array('class' => 'form-control', 'onkeyup'=>'number_price(this.value)')) }}
                            </div>
                        </div>
