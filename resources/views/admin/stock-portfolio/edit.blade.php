...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <form action="{{ route('admin.stock-portfolio.update',$item->id) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
            <div class="p-3">
                <div class="card p-4" style="height: 640px;overflow: auto;">
                    <div class="row" id="fa" role="tabpanel" aria-labelledby="farsi-tab">
        
                        <div class="col-12 form-group">
                            <label for="name" class="form-label text-success">+ افزودن توضیحات بیشتر فارسی ( این متن بالای همه ی توضیحات قرار میگیرد ) :</label>
                            <textarea class="form-control textarea " id="new_text" name="new_text"></textarea>
                        </div>
                        ...
                        <div class="col-12 form-group">
                            <label for="name" class="form-label">* توضیحات فارسی اصلی :</label>
                            <textarea class="form-control textarea " id="text" name="text">{{ $item->text }}</textarea>
                        </div>

                        @foreach ($item->children as $index => $children)
                            <div class="col-12 form-group">
                                <div class="float-right">
                                    <a href="javascript:void(0);" onclick="del_row('{{$children->id}}')" class="badge bg-danger mx-2" title="حذف"><i class="fa fa-trash"></i></a>
                                </div>
                                <label for="name" class="form-label">توضیحات الحاقی - {{$index+1}} :</label>
                                <textarea class="form-control textarea" id="text{{$children->id}}" name="text{{$children->id}}">{{ $children->text }}</textarea>
                            </div>
                        @endforeach
        ...
        function del_row(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    location.href = '{{url('/')}}/admin/stock-portfolio/force/children/destroy/'+id;
                }
            })
        }
    </script>
